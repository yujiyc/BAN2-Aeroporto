...
public class InsertEmpregado extends JPanel {
    private Sistema sistema = Sistema.getSistema();
    int index = 0;

    public InsertEmpregado() throws SQLException, SelectException {
        ...
        button.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                empregado.setNome(texto_nome.getText());
                empregado.setTelefone(texto_endereco.getText());
                empregado.setSalario(Double.parseDouble(texto_salario.getText()));
                empregado.setEndereco(texto_endereco.getText());
                for(int j=0;j<lista_sindicatos.size();j++){
                    if(array_sindicatos[j] == comboBox.getSelectedItem().toString()){
                        index = j;
                    }
                }
                empregado.setSindicato(sindicatos.get(index));
                
                try {
                    sistema.insereEmpregado(empregado);
